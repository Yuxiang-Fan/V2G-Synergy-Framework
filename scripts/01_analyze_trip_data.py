...
# 动态挂载项目根目录到 sys.path，确保正确导入 src 模块
...
def main():
    ...
    print(f"正在读取 NHTS 原始数据: {nhts_path}")
    
    try:
        df_trips = get_nhts_trip_data(nhts_path)
    except FileNotFoundError as e:
        print(f"\n[Error] {e}")
        print("请检查开源 NHTS 数据是否就绪，并确认 src/config.py 中的 DATA_PATHS 配置。")
        sys.exit(1)

    print(f"成功加载 {len(df_trips)} 条有效出行记录。")

    print("正在拟合出发时间 PMF 与行驶距离 KDE...")
    ...
    print(f"KDE Silverman 带宽: {kde_bandwidth:.4f}")
